...
//  管理者モード
// ════════════════════════════════════════════════════════
// 管理者判定＝書き込みトークン保有（2026-07-08）。旧パスワード方式は廃止。
// トークンはソースに含めず、管理者ログインで入力→localStorage(kv_write_token)にのみ保持。
// ════════════════════════════════════════════════════════

function isAdminMode() {
  // 管理者＝この端末に書き込みトークンを保有している（＝データ取得/保存が可能）。トークンはlocalStorageのみ。
  // ※旧・開発用トグル(kv_dev_admin)の優先判定は廃止（ログアウトが効かなくなる不具合の原因だったため）。
  return !!localStorage.getItem('kv_write_token');
}
...
function applyModeUI() {
  ...
  if (devBtn) devBtn.classList.add('hidden');   // 開発用トグルは常に隠す
  // ★管理者ログインボタンは閲覧者に見せない＝常に隠す。ログインは秘密の入口（URL #admin / ロゴ5回タップ）からのみ。
  if (loginBtn)  loginBtn.classList.add('hidden');
  if (badge)  badge.classList.toggle('hidden', !isAdmin);      // 管理者のときだけバッジ表示
  if (logoutBtn) logoutBtn.classList.toggle('hidden', !isAdmin); // 管理者のときだけログアウト表示
  if (isAdmin && typeof _renderAccessCounter === 'function') { try { _renderAccessCounter(); } catch (e) {} }
  if (typeof kvxApplyShell === 'function') kvxApplyShell();   // 管理者⇔閲覧者でシェル外観を即同期（閲覧者化で即除去）
}

/** 管理者ログインの秘密の入口を仕込む：①URLに #admin ②ヘッダーのロゴを5回タップ。閲覧者には何も見えない。 */
function _wireSecretAdminEntry() {
  // ① URLハッシュ #admin / #kanri でログインを開く（ブックマーク可）。開いたらハッシュは消す。
  const _hashOpen = () => {
  ...
  // ② ヘッダーのロゴ（🐴）を2秒以内に5回タップでログインを開く（スマホ用の隠し入口）
...
async function doAdminLogin() {
  ...
  // Worker にトークンを検証（存在しないID DELETE＝0行・データは変わらない。204=有効/401=無効）
  let ok = false;
  try {
    const res = await fetch(`${WORKER_URL}/rest/v1/keiba_races?id=eq.__auth_probe__`, {
      method: 'DELETE', headers: { 'X-Write-Token': token },
    });
    ok = (res.status !== 401 && res.status !== 403);
  } catch (e) { ok = false; }
  if (btn) { btn.disabled = false; if (btn.dataset._l) btn.innerHTML = btn.dataset._l; }
  if (ok) {
    localStorage.setItem('kv_write_token', token);
    closeAdminLogin();
    applyModeUI();
...
function adminLogout() {
  ...
  localStorage.removeItem('kv_dev_admin');   // 旧・開発用トグルの残骸も消す（ログアウトを確実に効かせる）
...
// 即時適用（DOMContentLoadedより早く実行されるケース対応）
(function(){ applyModeUI(); })();
